...
async function createTranslationQueues<TContext>(config: TranslationQueueSetupConfig<TContext>) {
  ...
  const browserTranslateQueueConfig = getBrowserTranslateQueueConfig(config.requestQueueConfig)
  ...
  return { requestQueue, batchQueue, browserTranslateRequestQueue }
}
...
export async function setUpWebPageTranslationQueue() {
  ...
  const { requestQueue, batchQueue, browserTranslateRequestQueue } = await createTranslationQueues({
    requestQueueConfig,
    batchQueueConfig,
    promptResolver: getTranslatePrompt,
  })
  let activeRequestQueueConfig = requestQueueConfig
  ...
  onMessage("enqueueTranslateRequest", async (message) => {
    ...
    if (shouldUseBatchQueue(providerConfig)) {
      ...
      const queue = isNonAPIProvider(providerConfig.provider) ? browserTranslateRequestQueue : requestQueue
      result = await queue.enqueue(thunk, scheduleAt, hash)
    }
  ...
  onMessage("setTranslateRequestQueueConfig", (message) => {
    const { data } = message
    activeRequestQueueConfig = {
      ...activeRequestQueueConfig,
      ...data,
    }
    requestQueue.setQueueOptions(data)
    browserTranslateRequestQueue.setQueueOptions(getBrowserTranslateQueueConfig(activeRequestQueueConfig))
  })
