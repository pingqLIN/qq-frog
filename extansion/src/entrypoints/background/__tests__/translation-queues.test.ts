...
describe("translation queue helpers", () => {
  ...
  it(
    ...
    async () => {
      const { getBrowserTranslateQueueConfig, shouldUseBatchQueue } = await import("../translation-queues")
      ...
      expect(getBrowserTranslateQueueConfig({ rate: 8, capacity: 60 })).toEqual({ rate: 2, capacity: 2 })
      expect(getBrowserTranslateQueueConfig({ rate: 1, capacity: 1 })).toEqual({ rate: 1, capacity: 1 })
    },
    15_000,
  )
