...
export class PageTranslationManager implements IPageTranslationManager {
  ...
  private static readonly MAX_CONCURRENT_ELEMENT_TRANSLATIONS = 3
  ...
  private elementTranslationQueue: Array<{ element: HTMLElement, walkId: string }> = []
  private activeElementTranslations = 0
  ...
  private stopInternal({ notify }: { notify: boolean }): void {
    ...
    this.elementTranslationQueue = []
    this.activeElementTranslations = 0
  ...
  private enqueueElementTranslation(element: HTMLElement, walkId: string): void {
    this.elementTranslationQueue.push({ element, walkId })
    this.drainElementTranslationQueue()
  }

  private drainElementTranslationQueue(): void {
    if (!this.isPageTranslating) {
      this.elementTranslationQueue = []
      this.activeElementTranslations = 0
      return
    ...
    while (
      this.activeElementTranslations < PageTranslationManager.MAX_CONCURRENT_ELEMENT_TRANSLATIONS
      && this.elementTranslationQueue.length > 0
    ) {
      const task = this.elementTranslationQueue.shift()
      if (!task || task.walkId !== this.walkId) {
        continue
      }

      this.activeElementTranslations++
      void this.translateQueuedElement(task.element, task.walkId)
    }
  }

  private async translateQueuedElement(element: HTMLElement, walkId: string): Promise<void> {
    try {
      const currentConfig = await getLocalConfig()
      if (!currentConfig) {
        logger.error("Global config is not initialized")
      ...
      await translateWalkedElement(element, walkId, currentConfig)
    }
    catch (error) {
      logger.warn("Failed to translate queued page element:", error)
    }
    finally {
      this.activeElementTranslations = Math.max(0, this.activeElementTranslations - 1)
      this.drainElementTranslationQueue()
    }
  }
  ...
  private async observerTopLevelParagraphs(container: HTMLElement, existingConfig?: Config): Promise<void> {
    ...
    if (container.hasAttribute("data-qq-frog-paragraph") && container.getAttribute("data-qq-frog-walked") === this.walkId) {
    ...
    const topLevelParagraphs = paragraphs.filter((el) => {
      const ancestor = el.parentElement?.closest("[data-qq-frog-paragraph]")
      // keep it if either:
  ...
  private collectParagraphElementsDeep(container: HTMLElement, walkId: string): HTMLElement[] {
    ...
    const collectFromContainer = (root: HTMLElement | Document | ShadowRoot) => {
      const elements = root.querySelectorAll<HTMLElement>(`[data-qq-frog-paragraph][data-qq-frog-walked="${CSS.escape(walkId)}"]`)
