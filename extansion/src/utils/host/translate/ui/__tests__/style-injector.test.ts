...
async function loadStyleInjector() {
  ...
  vi.doMock("@/assets/styles/custom-translation-node.css?raw", () => ({
    default: "@import '@/assets/styles/host-theme.css';\n[data-qq-frog-custom-translation-style='blur'] { opacity: 0.75; }",
  }))
  vi.doMock("@/assets/styles/host-theme.css?raw", () => ({
    default: ":root { --qq-frog-primary: oklch(0.205 0 0); --qq-frog-brand: oklch(76.034% 0.12361 82.191); }",
  }))
  vi.doMock("@/assets/styles/translation-node-preset.css?raw", () => ({
    default: ".qq-frog-translated-content-wrapper { display: inline; }",
  }))
...
describe("style-injector", () => {
  ...
  it("injects preset styles into the document", async () => {
    ...
    const presetStyle = document.head.querySelector<HTMLStyleElement>("#qq-frog-preset-styles")
    expect(presetStyle).not.toBeNull()
    expect(presetStyle?.textContent).toContain(".qq-frog-translated-content-wrapper")
  ...
  it("uses adoptedStyleSheets for document preset styles when available", async () => {
    ...
    expect(document.adoptedStyleSheets[0]?.cssRules[0]?.cssText).toContain("--qq-frog-brand")
    expect(document.head.querySelector("#qq-frog-preset-styles")).toBeNull()
  })
  ...
  it("injects preset styles into shadow roots with :host variables", async () => {
    ...
    const presetStyle = shadow.querySelector<HTMLStyleElement>("#qq-frog-preset-styles")
    expect(presetStyle).not.toBeNull()
    expect(presetStyle?.textContent).toContain(".qq-frog-translated-content-wrapper")
  ...
  it("ensures preset styles exist before custom document CSS", async () => {
    ...
    const presetStyle = document.head.querySelector<HTMLStyleElement>("#qq-frog-preset-styles")
    const customStyle = document.head.querySelector<HTMLStyleElement>("#qq-frog-custom-styles")
  ...
  it("uses adoptedStyleSheets for document custom CSS when available", async () => {
    ...
    expect(document.head.querySelector("#qq-frog-custom-styles")).toBeNull()
  })
})
